...
export const generarReporteAcademico = (stats, detailedData, options = {}) => {
  ...
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('3. Detalle por Alumno', margin, y);
  y += 10;

  autoTable(doc, {
    startY: y,
    head: [['No. Control', 'Alumno', 'Bonus', 'Razón', 'Calificación', 'Estatus']],
    body: detailedData.map(d => [
      d.matricula,
      d.alumno,
      d.bonus_materia ? `+${d.bonus_materia}` : '0',
      d.justificacion || '',
      Number(d.resultado_final || 0).toFixed(2),
      d.estatus
    ]),
