...
export default function Resultados() {
  ...
  async function exportarPDF() {
    ...
    try {
      ...
      generarReporteAcademico(stats, detailedResponse.data, {
        filename: `Reporte_${grupo?.nombre}_${new Date().toISOString().split('T')[0]}.pdf`,
        titulo_reporte: `Reporte Académico de Grupo — ${grupo?.nombre}`
      })
  ...
  return (
    <div>
      ...
      <Modal open={!!modalBonus} onClose={() => setModalBonus(null)}
        title={modalBonus?.tipo === 'unidad' ? `Bonus de Unidad ${modalBonus.unidad_num}` : "Bonus de Materia"}
      >
        {modalBonus && (
