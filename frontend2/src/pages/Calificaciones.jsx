...
export default function Calificaciones() {
  ...
  // ── Calcular promedio parcial dinámico para un alumno ─────────────────────
  async function calcularParcial(inscripcionId) {
    try {
      const r = await resApi.dinamico(inscripcionId, parseInt(unidadId))
      setResultados(prev => ({ 
        ...prev, 
        [inscripcionId]: { 
          final: r.data.resultado_final,
          bonus: r.data.bonus_aplicado
        } 
      }))
    } catch { }
  }
  ...
  async function guardarTodo() {
    ...
    try {
      ...
      reloaded.data.forEach(r => {
        if (!r.pendiente) calcularParcial(r.inscripcion_id)
      })
  ...
  const completados = registros.filter(r => !r.pendiente || edited[r.inscripcion_id]).length

  // Adaptación para el array devuelto por el backend
  const totalPendUnidad = Array.isArray(captura) ? captura.length : 0
  const maxPossible = acts.length * (numAlumnos || registros.length || 1)
  const pctUnidad = Array.isArray(captura) && maxPossible > 0
    ? Math.max(0, ((maxPossible - captura.length) / maxPossible) * 100)
    : 0
  const unidadCompleta = totalPendUnidad === 0 && captura !== null && acts.length > 0 && (numAlumnos > 0 || registros.length > 0)
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={act ? `Actividad: ${act.tipo} ${act.descripcion ? '— ' + act.descripcion : ''} · Peso: ${act.ponderacion}%` : acts.length === 0 ? 'Sin actividades' : 'Seleccione una actividad'}
        actions={
      ...
      <div className="p-8 space-y-4">
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-semibold ${unidadCompleta ? 'text-emerald-800' : 'text-amber-800'}`}>
                {unidadCompleta
                  ...
                  : `Faltan ${totalPendUnidad} calificaciones en esta unidad`}
              </p>
        ...
        <Card className="p-4">
          <div className="flex items-center gap-2 overflow-x-auto pb-1">
            {acts.map((a, i) => {
              // Pendientes específicos de esta actividad adaptado a array
              const pendAct = Array.isArray(captura) ? captura.filter(d => d.actividad_id === a.id).length : null
              return (
                <button
                  key={a.id}
                  onClick={() => setActIdx(i)}
                ...
                >
                  {a.tipo}{a.descripcion ? ` — ${a.descripcion}` : ''}
                  <span className="opacity-70">{a.ponderacion}%</span>
                  {pendAct !== null && pendAct > 0 && (
                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2">
                              <select
                                ...
                                className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-white focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none flex-1"
                              >
                              ...
                              {['TAREA', 'PROYECTO', 'PRACTICA_LAB'].includes(act?.tipo) && r.total_entregas > 0 && (
                                <button
                                  onClick={() => handleVerEntregas(r)}
                            ...
                            {isNP
                              ...
                              : (
                                <input
                                  ...
                                  onBlur={() => calcularParcial(r.inscripcion_id)}
                                  placeholder="—"
