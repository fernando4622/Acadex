...
export default function GrupoDetalle() {
  ...
  const ACT_ICONS = {
    EXAMEN: { icon: FileText, color: 'text-blue-500', bg: 'bg-blue-50' },
    TAREA: { icon: BookOpen, color: 'text-emerald-500', bg: 'bg-emerald-50' },
    PROYECTO: { icon: Layers, color: 'text-purple-500', bg: 'bg-purple-50' },
    PRACTICA_LAB: { icon: Beaker, color: 'text-orange-500', bg: 'bg-orange-50' },
    FORO: { icon: MessageSquare, color: 'text-cyan-500', bg: 'bg-cyan-50' },
    PARTICIPACION: { icon: User, color: 'text-rose-500', bg: 'bg-rose-50' },
    ASISTENCIA: { icon: CheckSquare, color: 'text-slate-500', bg: 'bg-slate-50' },
  }

  useEffect(() => {
    tiposActividadApi.listar().then(res => setTiposActividad(res.data)).catch(err => console.error("Error al cargar tipos de actividad", err))
  }, [])
  ...
  async function crearActividad(e) {
    ...
    try {
      await actividadesApi.crear(modalAct, {
        ...
        orden: parseInt(formAct.orden),
        fecha_apertura: formAct.fecha_apertura || null,
        fecha_cierre: formAct.fecha_cierre || null,
      })
      setModalAct(null)
      setFormA({ tipo: '', descripcion: '', ponderacion: '', orden: 1, fecha_apertura: '', fecha_cierre: '' })
  ...
  // ── Eliminar actividad ────────────────────────────────────────────────────
  async function eliminarActividad(actId, unidadId) {
    if (!confirm('¿Eliminar esta actividad? Si tiene calificaciones registradas se hará baja lógica.')) return
    try {
      const r = await actividadesApi.eliminar(actId)
  ...
  const puedeGestionarAlumnos = isAdmin || isDocente
  ...
  const alumnosDisponibles = todosAlumnos.filter(a => {
    ...
    const no_control = normalize(a.no_control)
    return fullName.includes(search) || no_control.includes(search)
  })
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            {(isAdmin || isDocente) && (
              <Btn variant="secondary" size="sm" onClick={() => navigate(`/grupos/${id}/resultados`)}>
        ...
        {statsGrupo && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            <StatCard
              ...
              value={statsGrupo.promedio_grupo ? statsGrupo.promedio_grupo.toFixed(1) : '--'}
              icon={BarChart2}
                ...
                <div className="border-t border-gray-100 px-5 pb-5 pt-4 space-y-4">
                          ...
                          <tr key={a.id} className="group">
                            <td className="py-2.5">
                              <div className="flex flex-col">
                                <div className="flex items-center gap-2">
                                  {(() => {
                                    const config = ACT_ICONS[a.tipo] || { icon: ClipboardList, color: 'text-gray-400', bg: 'bg-gray-50' }
                                  ...
                                  <div>
                                    <p className="font-medium text-gray-800 leading-none">{tiposActividad[a.tipo] || a.tipo}</p>
                                    {a.descripcion && <p className="text-[11px] text-gray-400 mt-1">{a.descripcion}</p>}
                                  </div>
                                </div>
                                {a.fecha_cierre && (
                                  <span className="text-[10px] text-gray-400 mt-1 ml-9">Cierre: {new Date(a.fecha_cierre).toLocaleString()}</span>
                                )}
                              </div>
                            </td>
                            <td className="py-2.5 text-right">
                            ...
                            {isDocente && !isAdmin && !cerrada && (
                              <td className="py-2.5 text-right">
                                <button
                                  onClick={() => eliminarActividad(a.id, u.id)}
                  ...
                  {isDocente && !isAdmin && !cerrada && suma < 100 && (
                    <Btn
                      variant="ghost"
                      size="sm"
                      onClick={() => { setModalAct(u.id); setError(null) }}
                    >
            ...
            <form onSubmit={crearActividad} className="space-y-4">
                ...
                >
                  ...
                  {Array.isArray(tiposActividad) && tiposActividad.map(t => (
                    <option key={t} value={t}>
                      {t === 'PRACTICA_LAB' ? 'Práctica de Lab'
                        : t === 'PARTICIPACION' ? 'Participación'
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input
                  ...
                  type="datetime-local"
                  value={formAct.fecha_apertura}
                  onChange={e => setFormA(f => ({ ...f, fecha_apertura: e.target.value }))}
                />
                <Input
                  label="Fecha de cierre"
                  type="datetime-local"
                  value={formAct.fecha_cierre}
                  onChange={e => setFormA(f => ({ ...f, fecha_cierre: e.target.value }))}
                />
              </div>
            ...
            {alumnosDisponibles.length === 0 ? (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-mono">No. Control: {a.num_control} {a.no_control && `${a.no_control}`}</p>
                </div>
              </button>
            ))}
          </div>
