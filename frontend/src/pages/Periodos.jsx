...
import { Btn, Input, Modal, Drawer, Table, Badge, ErrorMsg, Toast, EmptyState, Spinner, StatCard, ConfirmDialog } from '../components/ui'
...
export default function Periodos() {
  ...
  const [confirmActivar, setConfirmActivar] = useState(null) // { id, periodoActual }
  ...
  async function cargar() {
    try {
      ...
      const normalized = r.data.map(p => {
        let st = p.estado?.toUpperCase() || 'CERRADO'
        return {
          ...p,
          estado: st,
        }
  ...
  async function handleActivar(id) {
    try {
      // Primero intentar sin confirmación
      const res = await periodosApi.activar(id, { confirmar: false })
      if (res.data?.requiere_confirmacion) {
        // Hay otro activo — pedir confirmación al usuario
        setConfirmActivar({ id, periodoActual: res.data.periodo_activo_actual })
        return
      }
      await cargar()
      notify('Periodo activado exitosamente')
    } catch (err) { notify(err?.response?.data?.detail?.mensaje || 'Error al activar', 'error') }
  }

  async function confirmarActivacion() {
    if (!confirmActivar) return
    try {
      await periodosApi.activar(confirmActivar.id, { confirmar: true })
      setConfirmActivar(null)
      await cargar()
      notify('Periodo activado y anterior cerrado correctamente')
    } catch (err) { notify(err?.response?.data?.detail?.mensaje || 'Error al activar', 'error') }
  }
    ...
    },
    {
      ...
      render: (p) => (
        <div className="flex items-center gap-2">
          <Badge estado={p.estado?.toUpperCase()} />
        </div>
      )
    },
    ...(isAdmin ? [{
  ...
  return (
    <div className="min-h-screen bg-slate-50/30">
      <PageHeader
        ...
        icon={Calendar}
        actions={isAdmin && (
          <Btn variant="white-gold" onClick={() => { setModal(true); setError(null) }}><Plus size={18} /> Nuevo Periodo</Btn>
        )}
      />

      <div className="p-8 max-w-7xl mx-auto space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard label="Periodo Vigente" value={items.find(p => p.estado?.toUpperCase() === 'ACTIVO')?.nombre || 'Ninguno'} sub={items.find(p => p.estado?.toUpperCase() === 'ACTIVO')?.codigo || '—'} icon={Calendar} color="institucional-dark" />
      ...
      <Drawer open={modal} onClose={() => setModal(false)} title="Nuevo Periodo Académico"
      ...
      <ConfirmDialog
        ...
        message={`Ya hay un periodo activo: "${confirmActivar?.periodoActual?.nombre}". Al confirmar, ese periodo quedará CERRADO y se activará el nuevo. ¿Deseas continuar?`}
