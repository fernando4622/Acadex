...
import { Card, Spinner, Badge, Btn, Modal, Drawer, Input, Toast } from '../components/ui'
...
export default function UsuariosAdmin() {
  ...
  const handleToggleActive = async (id) => {
    try {
      ...
      setToast({ type: 'success', message: 'Estado de usuario actualizado' })
      load()
    } catch (e) {
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      setToast({ type: 'error', message: e.response?.data?.detail?.mensaje || 'Error al crear usuario' })
    } finally {
      setCreating(false)
    }
  }
  ...
  const handleManageRole = async (user, rol, action) => {
    try {
      ...
      setToast({ type: 'success', message: `Rol ${rol} ${action === 'add' ? 'asignado' : 'removido'}` })
      load()
    } catch (e) {
      setToast({ type: 'error', message: 'Error al gestionar rol' })
    }
  }
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filtered.map(u => (
            <Card key={u.id} className="group hover:shadow-md transition-all border-slate-200 cursor-pointer" onClick={() => { setSelectedUser(u); setShowRoleModal(true); }}>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex gap-1">
                    <Btn
                      ...
                      onClick={(e) => { e.stopPropagation(); setSelectedUser(u); setShowRoleModal(true); }}
                    >
