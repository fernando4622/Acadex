  ...
  for (let d = 1; d <= daysInMonth(year, month); d++) {
    ...
    days.push(
      <div
        ...
        className={`h-10 w-10 flex items-center justify-center rounded-xl text-sm font-bold relative transition-all duration-300 ${isToday ? 'bg-darkerBlue text-white shadow-glow scale-110 z-10' :
              ...
              {kpis.map(g => (
                <div key={g.grupo_id} className="p-4 rounded-2xl bg-slate-50 border border-slate-100">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="text-right">
                      <span className={`text-lg font-black ${g.rendimiento_relativo === 'SOBRE_PROMEDIO' ? 'text-emerald-600' :
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {enCurso.map((c, i) => (
                <div key={i} className="p-5 rounded-2xl bg-indigo-50 border border-indigo-100">
                  <div className="flex justify-between items-start mb-2">
                    <p className="font-bold text-slate-900">{c.materia}</p>
                    <span className="shrink-0 inline-flex items-center text-[10px] font-black px-2.5 py-1 rounded-md bg-indigo-200 border border-indigo-200 text-indigo-700 tracking-wider uppercase">
                      {c.estado || 'En curso'}
                    </span>                  </div>
