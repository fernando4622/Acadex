...
async def migrate_safe():
    ...
    try:
        async with conn.transaction():
            ...
            print("2. Asegurando integridad referencial de plan_materia...")
            # Si hay grupos que apuntan a una combinacion materia_id + plan_estudio_id que NO existe en plan_materia,
            # la creamos automáticamente para no perder los grupos ni las calificaciones.
            await conn.execute("""
                INSERT INTO academ.plan_materia (plan_estudio_id, materia_id, clave, semestre, orden)
                SELECT DISTINCT g.plan_estudio_id, g.materia_id, 'MIG-' || g.materia_id || '-' || g.plan_estudio_id, 1, 1
                FROM academ.grupo g
                WHERE NOT EXISTS (
                    SELECT 1 FROM academ.plan_materia pm 
                    WHERE pm.materia_id = g.materia_id AND pm.plan_estudio_id = g.plan_estudio_id
                )
            """)

            print("3. Modificando tabla grupo...")
            ...
            # Ahora ningún grupo debería tener plan_materia_id en NULL
            ...
            print("4. Modificando tabla inscripcion...")
            ...
            """)
            # Si hay inscripciones sin periodo (huerfanas de grupo), borrar (muy raro)
